...
export default function PhaseEditModal(props: Props) {
  ...
  const [teams, setTeams] = useState<Array<any>>(
    props?.phase?.teams?.map((team: any) => ({
      ...
      players: team?.players.map((player: any) => player?.playerDetail),
    }))
  );
  const editPhaseRequest = useRequest("put");
  const resetData = () => {
    setPlayers([]);
    setName("");
    setTeams([]);
  ...
  const submitData = () => {
    editPhaseRequest({
      url: `/tournament/update/phase/${props.tournamentKey}/${props?.phase?.key}`,
